# Canonical Ruleset of PHPSymphony

## Introduction

These rules codify the **Magnetar Canonical Project Model** standard for the PHPSymphony implementation. The entire project must comply with these rules unless a formal exception is documented in `BITACORA.md`.

## Naming Conventions

*   **Repositories:** `magnetar-phpsymphony` (Context only).
...
*   `projects/PHPSymphony.project.yml`.
...
**Transitions:** `planned` → `ready` → `in_progress` → `in_review` → `done`.
...
2.  **Assessment:** Update risks in `STATUS.md`, note mitigations in `BITACORA.md`.
3.  **Escalation:** Escalate if not resolved within 1 business day.
4.  **Resolution:** Document solution in `BITACORA.md`, update blocker to `resolved`.
5.  **Retrospective:** Capture lessons learned.

## Documentation Discipline

*   **BITACORA.md:** Chronological log of ALL state changes, decisions, exceptions.
*   **STATUS.md:** Update daily or after each PR merge.
*   **PLAN.md:** Source of truth for milestones/tasks.

## AI Agent Responsibilities

*   Parse project YAML before acting.
*   Do not open PRs unless task is `in_review`.
*   Document assumptions in `BITACORA.md`.

## Compliance and Enforcement

*   CI validates presence and structure of required files.
